...
const integration = { skip: baseUrl ? false : "Set TEST_BASE_URL to a local OpsDone server." };

let requestNumber = 1;
async function post(body) {
  return fetch(new URL("/api/process-check", baseUrl), {
    method: "POST",
    headers: {
...
test("process check rejects primitive JSON bodies", integration, async () => {
  for (const body of ["null", "[]", "42", '"text"']) {
    ...
    assert.deepEqual(await response.json(), { ok: false, error: "Ungültige Anfrage." });
  }
});

test("process check rejects payloads over the actual body limit", integration, async () => {
  const response = await post(JSON.stringify({ process: "x".repeat(17_000) }));
  assert.equal(response.status, 413);
  assert.equal((await response.json()).ok, false);
});

test("process check validates consent and required fields", integration, async () => {
  const noConsent = await post({
    name: "Ada Lovelace", email: "ada@example.com", company: "Example GmbH",
    process: "Reklamationen", consent: false,
  });
  ...
  const unexplainedOther = await post({
    name: "Ada Lovelace", email: "ada@example.com", company: "Example GmbH",
    process: "Anderer Prozess", message: "zu kurz", consent: true,
  });
...
test(
  ...
  {
    skip: !baseUrl || process.env.TEST_EXPECT_MISSING_INFRA !== "1"
